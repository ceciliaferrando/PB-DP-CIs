...
def plotIntervalsOld(path, dist, mode, epsilon):
    ...
    for method in methods:
        ...
        for N in Ns: 
            ...
            #results = results[:,0]
            #results = np.load(path+ "results_"+ dist +"_N" + str(N) +"_epsilon" + epsilon +"_analytic" + method +".npy", allow_pickle=True)
    ...
    for i,method in enumerate(methods):
        print(i, method)
        dg = draw_dodge(ax.errorbar, ["n=" + str(N) for N in Ns], ms[method], yerr = z_values[95]*sds[method], ax=ax, 
