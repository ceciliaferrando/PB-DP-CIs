...
def generate_ols_data(N, D, true_beta, gamma, delta, zeta):
    ...
    #discard data points that violate the bound on Y, if any (given bounds are conservative)
...
def coverage_test_single_trial(args):
    ...
    dist_std = np.sqrt(1.0 * sigma_sq_est / XtX[args.test_d][args.test_d])
...
parser.add_argument('--num_bootstraps', type=int, default=500, help='Number of experiments to compute quantile')
