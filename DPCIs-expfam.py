...
def parametricBootstrap(distribution, N, theta_vector, B, sensitivity, noise_scale, clipmin, clipmax, clip, coverage, rho):
    ...
    return(theta_tildas, theta_tildas_naive, theta_tildas_basic, fishInfInvSqrt, fishInfInvSqrtCorr,
...
def CIs(distribution, theta_vector, N, B, noise_scale, mode, T, cliplo, cliphi, rho):
    ...
    for coverage in list_ci_levels:
        ...
        for t in range(T):
            clip = True
            (theta_tildas, theta_tildas_naive, theta_tildas_basic, fishInfInvSqrt, fishInfInvSqrtCorr, fishInfInvSqrtNP,
             theta_priv, theta_basic, estimated_bias) = parametricBootstrap(distribution, N, theta_vector, B,
    ...
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) + '.npy'
    ...
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) + '_DEBIASED.npy'
    np.save('results_' + name_suffix, results_db)
    np.save('upperfailures_' + name_suffix, list_upper_failures_db)
    np.save('lowerfailures_' + name_suffix, list_lower_failures_db)
    np.save('widths_' + name_suffix, widths)
    
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) +  '_NAIVE.npy'
    ...
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) +  '_BASIC.npy'
    ...
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) +  '_FISHER.npy'
    ...
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) +  '_FISHERNP.npy'
    ...
    name_suffix = distribution + '_' + 'N'+str(N) + '_' + 'epsilon'+str(noise_scale) + '_' + mode + '_clampT_' + str(clipmax) +  '_FISHERCORR.npy'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--d', type=str, default='poisson', help='distribution (poisson, gaussian, gamma, gaussian2)')
    ...
    parser.add_argument('--cliphi', type=bool, default=6, help='clip upperbound')
